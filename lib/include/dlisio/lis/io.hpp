 ...
 * store the LR's from one Logical File. It's main purpose is to give random
 * access to LR's within a LF.
 *
 * Implicit records (normal data and alternate data) are store separately from
...
public:
    ...
    /** Map iflr's to their correct Data Format Specification Record (DFSR)
    *
    * Given a DFRS, implicits_of creates an iterable of implicit records
    * following the DFRS, terminated by either a new DFSR or End-of-Index.
    *
    * LIS79 do not explicitly mention the support for multiple DFSR or logsets,
    * but states:
    *
    * > The Data Record is the key to the LIS format as it defines the format
    * of data in a data record that [1].
    *
    ...
    *
    * [1] spec ref: LIS70 ch 3.3.2
    * [2] A common pattern found in real-world LIS files is that the same DFS
    *     record is recorded twice, most likely for redundancy. However, this
    *     usecase is not explicitly defined in LIS79. Following the spec to the
    *     point will mean that DFSR1 (from example) will count towards it's own
    *     (empty) logset.
    *
    *
 ...
 *      -----------------------------------------------------------
 *
 * In short, a PR is a mechanic for segmenting LRs into smaller chunks. LR's
 * _always_ start at the begining of a new PR, and may span multiple. Whether
 * or not a LR spans multiple LRs is described by the succesor/predeccesor
 ...
 *           ------------------------------------------------
 *
 * This means that the 3 PR's from the first illustration encapsulates 2 LRs.
 * The first LR spans PR1-PR2, and the second LR is contained in PR3. Note that
 * the LR header is *not* repeated in each PR when the LR spans multiple PRs.
 */
struct iodevice : public dlisio::stream {
public:
    explicit iodevice( lfp_protocol* );

    /* Read the physical header from the file. UB if position is not at
     * start of header [1]
     *
    ...
    /** Create a index of all the logical records in the file
     *
     * The returned record_index is a light-weigth logical record index which
     ...
     * is until index_records() is called. If the file is truncated, it's size
     * is unknown.
     *
...
/** Factory function for creating a iodevice
 *
 ...
 * until there are no more. Because lis in it self doesn't provide any clues to
 ...
 * */
iodevice open( const std::string&, std::int64_t, bool ) noexcept (false);
